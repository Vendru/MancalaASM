.data
	SEEDS_INIT: .word 4
	poco_p1: .word 24 # Ã­ndice 6*4 bytes
	...
	msg_sem_sementes: .asciz "Esta cavidade nÃ£o estÃ¡ semeada, escolha uma com sementes.\n"
	msg_invalida: .asciz "Esta cavidade nÃ£o Ã© vÃ¡lida, tente novamente.\n"
	
	msg_p1: .asciz "      --> JOGADOR 1\n"
	msg_p2: .asciz "      <-- JOGADOR 2\n"
	msg_vitoria_p1: .asciz "\nVitÃ³ria do Player 1!"
	msg_vitoria_p2: .asciz "\nVitÃ³ria do Player 2!"
	msg_empate: .asciz "\nO Empate!"
...
.text
	...
	# s2: offset poÃ§o P1 (24)
	# s3: offset poÃ§o P2 (52)
	
main:
	la s0, tabuleiro #carrega o endereÃ§o inicial do tabuleiro
...
main_loop:	
	...
	li t1, 1
	beq a0, t1, fim_jogo # se o a0 for 1 Ã© pq o jogo acabou
...
init_loop:
	...
	mul t3, t2, t5 # multiplica o indice por 4 para o addi, o deslocamento Ã© de 4 em 4 bytes
	# verifica se algum dos deslocamentos for igual aos dos poÃ§os
	beq t3, s2, init_poco
	beq t3, s3, init_poco
	add t4, s0, t3 # t4 recebe o endereÃ§o inicial do tabuleiro + deslocamento
	sw t0, 0(t4) # carrega o seeds init na cavidade -> 4 sementes
	j init_continua
	
init_poco:
	add t4, s0, t3
	sw zero, 0(t4) #como Ã© poÃ§o preenche com nada = 0
...
mostrar_tabuleiro:
	#addi -4 salva um espaÃ§o de 4 bytes para o ra
	addi sp, sp, -4
	sw ra, 0(sp)
	...
	li t0, 12 #indice t0 que comeÃ§a em 12
...
print_loop_p2:
	...
	add t3, s0, t2 #deslocamento + endereÃ§o base = posiÃ§Ã£o desejada
...
print_espacos_meio_fim:
	...
	add t0, s0, s2        # EndereÃ§o poÃ§o P1
	lw a0, (t0)
	li a7, 1
	ecall
...
pede_jogada:
	...
	#verifica se estÃ¡ entre 0 e 5 ou 7 e 12(depende se veio de play_p1 ou p2)
...
distribui_sementes:
	mv t0, a0             # t0 = Ã­ndice atual
	mv t1, a1             # t1 = sementes na mÃ£o
